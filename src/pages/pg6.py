...
dash.register_page (__name__,
                    path = '/agua_e_saneamento', #  is home page and it represents the url
                    name = "Água e saneamento", # name of page, commonly used as name of link
                    title='Água e saneamento',  # title that appears on browser's tab
                    #image='pg1.png',  # image in the assets folder
                    description="Índices e variações do sector da água e saneamento"

                    )

# Fazer leitura da dataframe do industria
agua_df = pd.read_csv('resultados por sector de actividade.csv', low_memory=False, encoding="iso-8859-1",  dtype= {'CAE': np.object_})

agua_df = agua_df[agua_df["Sector de actividade"] == "Água e saneamento"]

#comercio_df = pd.read_csv('resultados_comercio.csv', low_memory=False, encoding="iso-8859-1")


# melting a dataframe do comercio
agua_df_melted = pd.melt(agua_df, id_vars =['CAE', 'Descrição do código da CAE',\
...
agua_df_melted["Mes/Ano"]= agua_df_melted["Meses"]+"-"+[x[2:] for x in agua_df_melted["Ano"].astype(str)]
...
layout = html.Div(
    [
        html.H6('Resultados do índice no sector da água e saneamento', style = {'font-family': 'Source Sans Pro', 'font-size': '16px',
            ...
            [
                dbc.Col([
                        ...
                        dbc.Label('Selecione o ano:', style = {'font-family': 'Source Sans Pro', 'font-size': '12px',\
                                                               'justifyContent': 'right', "text-justify": "inter-word",
                                                               'textAlign': 'justify',"font-weight": "bold" }),
                        dcc.Dropdown(id='ano_id_agua', value=agua_df["Ano"].max(),
                        placeholder='Anos',
                        multi=False,
                        options=[{'label': ano, 'value': ano}
                                  for ano in agua_df["Ano"].unique()]),
                        html.Hr(),
                        ...
                        dcc.RadioItems(id='resultados_id_agua', value=list(agua_df["Tipo de resultados"].unique())[0], 
                                       options=[{'label': html.Div(tipo,style={'color': '', 'padding-left': 10,\
                                                            'font-family': 'Source Sans Pro', 'font-size': '12px'}), 'value': tipo, }
                                  for tipo in agua_df['Tipo de resultados'].unique()], inline=False,
                        ...
                        dcc.RadioItems(id='nivel_id_agua', #value='Secção', 
                                   options=[{'label': html.Div(categoria,style={'color': '', 'padding-left': 10,\
                                                        'font-family': 'Source Sans Pro', 'font-size': '12px'}), 'value': categoria, }
                              for categoria in list(filter(lambda x: x != "Grupo", agua_df['Nível de codificação'].unique()))], inline=False,
                        ...
                        html.Hr(),
                        dcc.Checklist(id='nivel_options_id_agua', className="checklist" ),
                ...
                dbc.Col(
                        ...
                        dbc.Row([
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_vvn_c_agua', style={'width': '100%','display': 'inline-block',},
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_nps_c_agua', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
                        ...
                        dbc.Row([
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_rem_c_agua', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_horas_c_agua', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
            ...
            [
                ...
                dbc.Col(
                    [
                        ...
                        dash_table.DataTable(id="tabela_descricao_id_agua",
                                  columns=[{"name":i, "id":i,
                                  "selectable":True, 
                                  } for i in agua_df[["CAE", "Descrição do código da CAE"]].columns 
                                  ], data= "",                                  
            ...
            [
                dbc.Col(
                    [
                        ...
                        dcc.Dropdown(id='nivel_id_c_agua',   
                        placeholder='Selecione o nível',
                        multi=False,
                        options=[{'label': nivel, 'value': nivel}
                                  for nivel in agua_df["Nível de codificação"].unique()]),
                        
                        html.Br(), 
                        html.Br(), 

                        dcc.Dropdown(id='tipo_resultados_id_c_agua',   
                        placeholder='Selecione o tipo de resultados',
                        multi=False,
                        options=[{'label': tipo_resultados, 'value': tipo_resultados}
                                  for tipo_resultados in agua_df['Tipo de resultados'].unique()]),

                        html.Br(),
                        html.Br(), 

                        dcc.Dropdown(id='categoria_id_c_agua',
                        placeholder='Selecione a categoria de resultados',
                        multi=False,
                        options=[{'label': categoria, 'value': categoria}
                                  for categoria in agua_df["Categoria"].unique()]),

                        html.Br(),
                        html.Br(), 

                        dcc.Dropdown(id='ano_id_c_agua',
                ...
                dbc.Col(
                    [
                        dash_table.DataTable(id="tabela_comercio_id_agua",
                                  columns=[{"name":i, "id":i,
                                  "selectable":True, "hideable":True,# "presentation":"dropdown"
                                  } \
                                    for i in list(filter(lambda i: i not in ["Sector de actividade", "Distribuição do peso da divisão e grupo"], agua_df.columns))
                                  ], data= agua_df.to_dict("records"),                                  
...
@callback(Output('nivel_options_id_agua', 'options'),
                Output('nivel_options_id_agua', 'inline'),
                Output('nivel_options_id_agua', 'inputStyle'),
                Output('nivel_options_id_agua', 'labelStyle'),  
                Input('nivel_id_agua', 'value'))
...
@callback(Output('descricao_id_agua', 'children'),
              Input('nivel_id_agua', 'value'),)
...
@callback(Output("tabela_descricao_id_agua", 'data'),
              Input('nivel_id_agua', 'value'),)
...
@callback(Output('ind_var_vvn_c_agua', 'figure'),
              Input('ano_id_agua', 'value'),
              Input('resultados_id_agua', 'value'), 
              Input('categoria_id_agua', 'value'),
              Input('nivel_id_agua', 'value'),        
              Input('nivel_options_id_agua', 'value'))

# Resultados do volume de negócios
def resultado_vvn(ano, resultados, categoria, nivel, opcoes):
    
    # Copiando a dataset
    df = agua_df_melted.copy()
...
@callback(Output('ind_var_nps_c_agua', 'figure'),
              Input('ano_id_agua', 'value'),
              Input('resultados_id_agua', 'value'), 
              Input('categoria_id_agua', 'value'),
              Input('nivel_id_agua', 'value'),        
...
@callback(Output('ind_var_rem_c_agua', 'figure'),
              Input('ano_id_agua', 'value'),
              Input('resultados_id_agua', 'value'), 
              Input('categoria_id_agua', 'value'),
              Input('nivel_id_agua', 'value'),        
...
@callback(Output('ind_var_horas_c_agua', 'figure'),
              Input('ano_id_agua', 'value'),
              Input('resultados_id_agua', 'value'), 
              Input('categoria_id_agua', 'value'),
              Input('nivel_id_agua', 'value'),        
...
@callback(Output("tabela_comercio_id_agua", 'data'),
              Input('nivel_id_c_agua', 'value'),
              Input('tipo_resultados_id_c_agua', 'value'), 
              Input('categoria_id_c_agua', 'value'),
              Input('ano_id_c_agua', 'value'))
