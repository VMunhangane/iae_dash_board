...
dash.register_page (__name__,
                    path = '/outros_servicos', #  is home page and it represents the url
                    name = "Outros serviços", # name of page, commonly used as name of link
                    title='Outros serviços',  # title that appears on browser's tab
                    #image='pg1.png',  # image in the assets folder
                    description="Índices e variações do sector de outros serviços"

                    )

# Fazer leitura da dataframe do industria
outros_servicos_df = pd.read_csv('resultados por sector de actividade.csv', low_memory=False, encoding="iso-8859-1", dtype= {'CAE': np.object_})

outros_servicos_df = outros_servicos_df[outros_servicos_df["Sector de actividade"] == "Outros serviços"]

#comercio_df = pd.read_csv('resultados_comercio.csv', low_memory=False, encoding="iso-8859-1")


# melting a dataframe do comercio
outros_servicos_df_melted = pd.melt(outros_servicos_df, id_vars =['CAE', 'Descrição do código da CAE',\
...
outros_servicos_df_melted["Mes/Ano"]= outros_servicos_df_melted["Meses"]+"-"+[x[2:] for x in outros_servicos_df_melted["Ano"].astype(str)]
...
layout = html.Div(
    [
        html.H6('Resultados do índice no sector de outros serviços', style = {'font-family': 'Source Sans Pro', 'font-size': '16px',
            ...
            [
                dbc.Col([
                        ...
                        dbc.Label('Selecione o ano:', style = {'font-family': 'Source Sans Pro', 'font-size': '12px',\
                                                               'justifyContent': 'right', "text-justify": "inter-word",
                                                               'textAlign': 'justify',"font-weight": "bold" }),
                        dcc.Dropdown(id='ano_id_ser', value=outros_servicos_df["Ano"].max(),
                        placeholder='Anos',
                        multi=False,
                        options=[{'label': ano, 'value': ano}
                                  for ano in outros_servicos_df["Ano"].unique()]),
                        html.Hr(),
                        ...
                        dcc.RadioItems(id='resultados_id_ser', value=list(outros_servicos_df["Tipo de resultados"].unique())[0], 
                                       options=[{'label': html.Div(tipo,style={'color': '', 'padding-left': 10,\
                                                            'font-family': 'Source Sans Pro', 'font-size': '12px'}), 'value': tipo, }
                                  for tipo in outros_servicos_df['Tipo de resultados'].unique()], inline=False,
                        ...
                        dcc.RadioItems(id='nivel_id_ser', #value='Secção', 
                                   options=[{'label': html.Div(categoria,style={'color': '', 'padding-left': 10,\
                                                        'font-family': 'Source Sans Pro', 'font-size': '12px'}), 'value': categoria, }
                              for categoria in list(filter(lambda x: x != "Grupo", outros_servicos_df['Nível de codificação'].unique()))], inline=False,
                        ...
                        html.Hr(),
                        dcc.Checklist(id='nivel_options_id_ser', className="checklist" ),
                ...
                dbc.Col(
                        ...
                        dbc.Row([
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_vvn_c_ser', style={'width': '100%','display': 'inline-block',},
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_nps_c_ser', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
                        ...
                        dbc.Row([
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_rem_c_ser', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_horas_c_ser', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
            ...
            [
                ...
                dbc.Col(
                    [
                        ...
                        dash_table.DataTable(id="tabela_descricao_id_ser",
                                  columns=[{"name":i, "id":i,
                                  "selectable":True, 
                                  } for i in outros_servicos_df[["CAE", "Descrição do código da CAE"]].columns 
                                  ], data= "",                                  
            ...
            [
                dbc.Col(
                    [
                        ...
                        dcc.Dropdown(id='nivel_id_c_ser',   
                        placeholder='Selecione o nível',
                        multi=False,
                        options=[{'label': nivel, 'value': nivel}
                                  for nivel in outros_servicos_df["Nível de codificação"].unique()]),
                        
                        html.Br(), 
                        html.Br(), 

                        dcc.Dropdown(id='tipo_resultados_id_c_ser',   
                        placeholder='Selecione o tipo de resultados',
                        multi=False,
                        options=[{'label': tipo_resultados, 'value': tipo_resultados}
                                  for tipo_resultados in outros_servicos_df['Tipo de resultados'].unique()]),

                        html.Br(),
                        html.Br(), 

                        dcc.Dropdown(id='categoria_id_c_ser',
                        placeholder='Selecione a categoria de resultados',
                        multi=False,
                        options=[{'label': categoria, 'value': categoria}
                                  for categoria in outros_servicos_df["Categoria"].unique()]),

                        html.Br(),
                        html.Br(), 

                        dcc.Dropdown(id='ano_id_c_ser',
                ...
                dbc.Col(
                    [
                        dash_table.DataTable(id="tabela_comercio_id_ser",
                                  columns=[{"name":i, "id":i,
                                  "selectable":True, "hideable":True,# "presentation":"dropdown"
                                  }\
                                    for i in list(filter(lambda i: i not in ["Sector de actividade", "Distribuição do peso da divisão e grupo"], outros_servicos_df.columns))
                                  ], data= outros_servicos_df.to_dict("records"),                                  
...
@callback(Output('nivel_options_id_ser', 'options'),
                Output('nivel_options_id_ser', 'inline'),
                Output('nivel_options_id_ser', 'inputStyle'),
                Output('nivel_options_id_ser', 'labelStyle'),  
                Input('nivel_id_ser', 'value'))

def opcoes_nivel_cae(nivel):
    df = outros_servicos_df.copy()
...
@callback(Output('descricao_id_ser', 'children'),
              Input('nivel_id_ser', 'value'),)
...
@callback(Output("tabela_descricao_id_ser", 'data'),
              Input('nivel_id_ser', 'value'),)
...
@callback(Output('ind_var_vvn_c_ser', 'figure'),
              Input('ano_id_ser', 'value'),
              Input('resultados_id_ser', 'value'), 
              Input('categoria_id_ser', 'value'),
              Input('nivel_id_ser', 'value'),        
              Input('nivel_options_id_ser', 'value'))

# Resultados do volume de negócios
def resultado_vvn(ano, resultados, categoria, nivel, opcoes):
    
    # Copiando a dataset
    df = outros_servicos_df_melted.copy()
...
@callback(Output('ind_var_nps_c_ser', 'figure'),
              Input('ano_id_ser', 'value'),
              Input('resultados_id_ser', 'value'), 
              Input('categoria_id_ser', 'value'),
              Input('nivel_id_ser', 'value'),        
...
@callback(Output('ind_var_rem_c_ser', 'figure'),
              Input('ano_id_ser', 'value'),
              Input('resultados_id_ser', 'value'), 
              Input('categoria_id_ser', 'value'),
              Input('nivel_id_ser', 'value'),        
...
@callback(Output('ind_var_horas_c_ser', 'figure'),
              Input('ano_id_ser', 'value'),
              Input('resultados_id_ser', 'value'), 
              Input('categoria_id_ser', 'value'),
              Input('nivel_id_ser', 'value'),        
...
@callback(Output("tabela_comercio_id_ser", 'data'),
              Input('nivel_id_c_ser', 'value'),
              Input('tipo_resultados_id_c_ser', 'value'), 
              Input('categoria_id_c_ser', 'value'),
              Input('ano_id_c_ser', 'value'))
