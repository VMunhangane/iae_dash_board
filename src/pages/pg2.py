...
# configurando o nome da tab
dash.register_page(__name__,
                   path="/alojamento_e_restauracao", # vai representar o URL da pagina
                   name="Alojamento e restauração", # title that appears on browser's tab
                   title="Alojamento e restauração", # title that appears on browser's tab
                   #image='pg1.png',  # image in the assets folder
                   description="Índices e variações no sector de alojamento e restauração")


# Page for the index results of trade

# Page for the index results of trade

# Fazer leitura da dataframe do comercio
alojamento_df = pd.read_csv('resultados por sector de actividade.csv', low_memory=False, encoding="iso-8859-1",  dtype= {'CAE': np.object_})

alojamento_df = alojamento_df[alojamento_df["Sector de actividade"] == "Alojamento e restauração"]

#comercio_df = pd.read_csv('resultados_comercio.csv', low_memory=False, encoding="iso-8859-1")


# melting a dataframe do comercio
alojamento_df_melted = pd.melt(alojamento_df, id_vars =['CAE', 'Descrição do código da CAE',\
...
alojamento_df_melted["Mes/Ano"]= alojamento_df_melted["Meses"]+"-"+[x[2:] for x in alojamento_df_melted["Ano"].astype(str)]
...
layout = html.Div(
    [
        html.H6('Resultados do índice no sector do alojamento e restauração', style = {'font-family': 'Source Sans Pro', 'font-size': '16px',
            ...
            [
                dbc.Col([
                        ...
                        dbc.Label('Selecione o ano:', style = {'font-family': 'Source Sans Pro', 'font-size': '12px',\
                                                               'justifyContent': 'right', "text-justify": "inter-word",
                                                               'textAlign': 'justify',"font-weight": "bold" }),
                        dcc.Dropdown(id='ano_id_alo', value=alojamento_df["Ano"].max(),
                        placeholder='Anos',
                        multi=False,
                        options=[{'label': ano, 'value': ano}
                                  for ano in alojamento_df["Ano"].unique()]),
                        html.Hr(),
                        ...
                        dcc.RadioItems(id='resultados_id_alo', value=list(alojamento_df["Tipo de resultados"].unique())[0], 
                                       options=[{'label': html.Div(tipo,style={'color': '', 'padding-left': 10,\
                                                            'font-family': 'Source Sans Pro', 'font-size': '12px'}), 'value': tipo, }
                                  for tipo in alojamento_df['Tipo de resultados'].unique()], inline=False,
                        ...
                        # dcc.RadioItems(list(comercio_df['Nível de codificação'].unique()), 'Secção', id='nivel_id', 
                        #         #        options=[{'label': html.Div(categoria,style={'color': '', 'padding-left': 10,\
                        #         #                             'font-family': 'Source Sans Pro', 'font-size': '12px'}), 'value': categoria, }
                        #         #   for categoria in comercio_df['Nível de codificação'].unique()], inline=False,
                        #           inputStyle={'cursor': 'pointer'}, 
                        #           labelStyle={"display": "flex", 'padding': '0.5rem', 'margin-left':'5px'}
                        #           ),

                        dcc.RadioItems(id='nivel_id_alo', #value='Secção', 
                                   options=[{'label': html.Div(categoria,style={'color': '', 'padding-left': 10,\
                                                        'font-family': 'Source Sans Pro', 'font-size': '12px'}), 'value': categoria, }
                              for categoria in list(filter(lambda x: x != "Grupo", alojamento_df['Nível de codificação'].unique()))], inline=False,
                        ...
                        html.Hr(),
                        dcc.Checklist(id='nivel_options_id_alo', className="checklist" ),
                ...
                dbc.Col(
                        ...
                        dbc.Row([
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_vvn_c_alo', style={'width': '100%','display': 'inline-block',},
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_nps_c_alo', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
                        ...
                        dbc.Row([
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_rem_c_alo', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_horas_c_alo', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
            ...
            [
                ...
                dbc.Col(
                    [
                        ...
                        dash_table.DataTable(id="tabela_descricao_id_alo",
                                  columns=[{"name":i, "id":i,
                                  "selectable":True, 
                                  } for i in alojamento_df[["CAE", "Descrição do código da CAE"]].columns 
                                  ], data= "",                                  
            ...
            [
                dbc.Col(
                    [
                        ...
                        dcc.Dropdown(id='nivel_id_c_alo',   
                        placeholder='Selecione o nível',
                        multi=False,
                        options=[{'label': nivel, 'value': nivel}
                                  for nivel in alojamento_df["Nível de codificação"].unique()]),
                        
                        html.Br(), 
                        html.Br(), 

                        dcc.Dropdown(id='tipo_resultados_id_c_alo',   
                        placeholder='Selecione o tipo de resultados',
                        multi=False,
                        options=[{'label': tipo_resultados, 'value': tipo_resultados}
                                  for tipo_resultados in alojamento_df['Tipo de resultados'].unique()]),

                        html.Br(),
                        html.Br(), 

                        dcc.Dropdown(id='categoria_id_c_alo',
                        placeholder='Selecione a categoria de resultados',
                        multi=False,
                        options=[{'label': categoria, 'value': categoria}
                                  for categoria in alojamento_df["Categoria"].unique()]),

                        html.Br(),
                        html.Br(), 

                        dcc.Dropdown(id='ano_id_c_alo',
                ...
                dbc.Col(
                    [
                        dash_table.DataTable(id="tabela_comercio_id_alo",
                                  columns=[{"name":i, "id":i,
                                  "selectable":True, "hideable":True,# "presentation":"dropdown"
                                  }\
                                    for i in list(filter(lambda i: i not in ["Sector de actividade", "Distribuição do peso da divisão e grupo"], alojamento_df.columns))
                                  ], data= alojamento_df.to_dict("records"),                                  
...
@callback(Output('nivel_options_id_alo', 'options'),
                Output('nivel_options_id_alo', 'inline'),
                Output('nivel_options_id_alo', 'inputStyle'),
                Output('nivel_options_id_alo', 'labelStyle'),  
                Input('nivel_id_alo', 'value'))

def opcoes_nivel_cae(nivel):
    df = alojamento_df.copy()
...
@callback(Output('descricao_id_alo', 'children'),
              Input('nivel_id_alo', 'value'),)
...
@callback(Output("tabela_descricao_id_alo", 'data'),
              Input('nivel_id_alo', 'value'),)
...
@callback(Output('ind_var_vvn_c_alo', 'figure'),
              Input('ano_id_alo', 'value'),
              Input('resultados_id_alo', 'value'), 
              Input('categoria_id_alo', 'value'),
              Input('nivel_id_alo', 'value'),        
              Input('nivel_options_id_alo', 'value'))

# Resultados do volume de negócios
def resultado_vvn(ano, resultados, categoria, nivel, opcoes):
    
    # Copiando a dataset
    df = alojamento_df_melted.copy()
...
@callback(Output('ind_var_nps_c_alo', 'figure'),
              Input('ano_id_alo', 'value'),
              Input('resultados_id_alo', 'value'), 
              Input('categoria_id_alo', 'value'),
              Input('nivel_id_alo', 'value'),        
...
@callback(Output('ind_var_rem_c_alo', 'figure'),
              Input('ano_id_alo', 'value'),
              Input('resultados_id_alo', 'value'), 
              Input('categoria_id_alo', 'value'),
              Input('nivel_id_alo', 'value'),        
...
@callback(Output('ind_var_horas_c_alo', 'figure'),
              Input('ano_id_alo', 'value'),
              Input('resultados_id_alo', 'value'), 
              Input('categoria_id_alo', 'value'),
              Input('nivel_id_alo', 'value'),        
...
@callback(Output("tabela_comercio_id_alo", 'data'),
              Input('nivel_id_c_alo', 'value'),
              Input('tipo_resultados_id_c_alo', 'value'), 
              Input('categoria_id_c_alo', 'value'),
              Input('ano_id_c_alo', 'value'))
