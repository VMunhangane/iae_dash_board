...
#from dash.dash_table.Format import Grouproup


# configurando o nome da tab
dash.register_page(__name__,
                   path="/comercio", # vai representar o URL da pagina
                   name="Comércio", # title that appears on browser's tab
                   title="Comércio", # title that appears on browser's tab
                   #image='pg1.png',  # image in the assets folder
                   description="Índices e variações do sector do comércio")
...
comercio_df = comercio_df[comercio_df["Sector de actividade"] == "Comércio"]

#comercio_df = pd.read_csv('resultados_comercio.csv', low_memory=False, encoding="iso-8859-1")


# melting a dataframe do comercio
comercio_df_melted = pd.melt(comercio_df, id_vars =['CAE', 'Descrição do código da CAE',\
...
comercio_df_melted["Mes/Ano"]= comercio_df_melted["Meses"]+"-"+[x[2:] for x in comercio_df_melted["Ano"].astype(str)]
...
layout = html.Div(
    [
        html.H6('Resultados do índice no sector do comércio', style = {'font-family': 'Source Sans Pro', 'font-size': '16px',
            ...
            [
                dbc.Col([
                        ...
                        dbc.Label('Selecione o ano:', style = {'font-family': 'Source Sans Pro', 'font-size': '12px',\
                                                               'justifyContent': 'right', "text-justify": "inter-word",
                                                               'textAlign': 'justify',"font-weight": "bold" }),
                        dcc.Dropdown(id='ano_id', value=comercio_df["Ano"].max(),
                        placeholder='Anos',
                        multi=False,
                        options=[{'label': ano, 'value': ano}
                                  for ano in comercio_df["Ano"].unique()]),
                        html.Hr(),
                        ...
                        dcc.RadioItems(id='resultados_id', value=list(comercio_df["Tipo de resultados"].unique())[0], 
                                       options=[{'label': html.Div(tipo,style={'color': '', 'padding-left': 10,\
                                                            'font-family': 'Source Sans Pro', 'font-size': '12px'}), 'value': tipo, }
                                  for tipo in comercio_df['Tipo de resultados'].unique()], inline=False,
                        ...
                        dcc.RadioItems(id='nivel_id', #value='Secção', 
                                   options=[{'label': html.Div(categoria,style={'color': '', 'padding-left': 10,\
                                                        'font-family': 'Source Sans Pro', 'font-size': '12px'}), 'value': categoria, }
                              for categoria in list(filter(lambda x: x != "Grupo", comercio_df['Nível de codificação'].unique()))], inline=False,
                        ...
                        html.Hr(),
                        dcc.Checklist(id='nivel_options_id', className="checklist" ),
                ...
                dbc.Col(
                        ...
                        dbc.Row([
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_vvn_c', style={'width': '100%','display': 'inline-block',},
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_nps_c', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
                        ...
                        dbc.Row([
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_rem_c', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
                            ...
                            dbc.Col(
                                ...
                                dcc.Graph(id='ind_var_horas_c', style={'width': '100%','display': 'inline-block',},
                                config = {'modeBarButtonsToRemove':["zoom2d", "select2d", "lasso2d", "resetscale2d"],
                                'modeBarButtonsToAdd' : ['toImage','pan2d', 'autoScaled2d', 'zoomIn2d', 'zoomOut2d'],
                                    },
            ...
            [
                ...
                dbc.Col(
                    [
                        ...
                        dash_table.DataTable(id="tabela_descricao_id",
                                  columns=[{"name":i, "id":i,
                                  "selectable":True, 
                                  } for i in comercio_df[["CAE", "Descrição do código da CAE"]].columns 
                                  ], data= "",                                  
            ...
            [
                dbc.Col(
                    [
                        ...
                        dcc.Dropdown(id='nivel_id_c',   
                        placeholder='Selecione o nível',
                        multi=False,
                        options=[{'label': nivel, 'value': nivel}
                                  for nivel in comercio_df["Nível de codificação"].unique()]),
                        
                        html.Br(), 
                        html.Br(), 

                        dcc.Dropdown(id='tipo_resultados_id_c',   
                        placeholder='Selecione o tipo de resultados',
                        multi=False,
                        options=[{'label': tipo_resultados, 'value': tipo_resultados}
                                  for tipo_resultados in comercio_df['Tipo de resultados'].unique()]),

                        html.Br(),
                        html.Br(), 

                        dcc.Dropdown(id='categoria_id_c',
                        placeholder='Selecione a categoria de resultados',
                        multi=False,
                        options=[{'label': categoria, 'value': categoria}
                                  for categoria in comercio_df["Categoria"].unique()]),

                        html.Br(),
                        html.Br(), 

                        dcc.Dropdown(id='ano_id_c',
                ...
                dbc.Col(
                    [
                        dash_table.DataTable(id="tabela_comercio_id",
                                  columns=[{"name":i, "id":i,
                                  "selectable":True, "hideable":True,# "presentation":"dropdown"
                                  }\
                                    for i in list(filter(lambda i: i not in ["Sector de actividade", "Distribuição do peso da divisão e grupo"], comercio_df.columns))
                                  ], data= comercio_df.to_dict("records"),                                  
...
@callback(Output('nivel_options_id', 'options'),
                Output('nivel_options_id', 'inline'),
                Output('nivel_options_id', 'inputStyle'),
                Output('nivel_options_id', 'labelStyle'),  
                Input('nivel_id', 'value'))
...
@callback(Output('descricao_id', 'children'),
              Input('nivel_id', 'value'),)
...
@callback(Output("tabela_descricao_id", 'data'),
              Input('nivel_id', 'value'),)
...
@callback(Output('ind_var_nps_c', 'figure'),
              Input('ano_id', 'value'),
              Input('resultados_id', 'value'), 
              Input('categoria_id', 'value'),
              Input('nivel_id', 'value'),        
...
@callback(Output('ind_var_rem_c', 'figure'),
              Input('ano_id', 'value'),
              Input('resultados_id', 'value'), 
              Input('categoria_id', 'value'),
              Input('nivel_id', 'value'),        
...
@callback(Output('ind_var_horas_c', 'figure'),
              Input('ano_id', 'value'),
              Input('resultados_id', 'value'), 
              Input('categoria_id', 'value'),
              Input('nivel_id', 'value'),        
...
@callback(Output("tabela_comercio_id", 'data'),
              Input('nivel_id_c', 'value'),
              Input('tipo_resultados_id_c', 'value'), 
              Input('categoria_id_c', 'value'),
              Input('ano_id_c', 'value'))
